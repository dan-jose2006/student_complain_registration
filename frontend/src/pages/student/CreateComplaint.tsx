        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              {reframePreview && (
                <div className="mt-3 rounded-2xl border border-violet-500/30 bg-gradient-to-br from-violet-500/5 via-purple-500/5 to-indigo-500/5 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-300">
                  {/* Header */}
...
export const CreateComplaint: React.FC = () => {
  ...
  const handleAnalyzeAI = async () => {
    ...
    setIsAnalyzing(true);
    try {
      info('Analyzing your issue with Groq LLaMA 3.3 AI...', 'AI Assistant');
      const analysis = await aiService.analyzeComplaint({ title, description, location });
      setAiSuggestion(analysis);
      success('AI categorized your issue and estimated priority level.', 'AI Triage Ready');
    } catch (err: any) {
    ...
    <div className="max-w-4xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
      ...
      <Card className="shadow-md border-border/80 bg-card rounded-3xl overflow-hidden">
        <CardHeader>
          <CardTitle className="text-lg">Issue Details</CardTitle>
          <CardDescription>
            Fill out the form below. You can also use CampusCare AI to automatically determine the category and urgency.
          </CardDescription>
        </CardHeader>

        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            {/* Title & AI analyze trigger */}
            <div className="space-y-2">
              <label className="text-xs font-bold text-foreground flex items-center gap-1.5">
                <FileText className="w-3.5 h-3.5 text-muted-foreground" />
                Issue Title <span className="text-destructive">*</span>
